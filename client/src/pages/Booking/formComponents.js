        ...
        <>
            ...
            {array2.map((item) => (
                <div className="mb-3" key={'input' + item.toString()}>
                    <Label key={'label' + item.toString()} className="m-0" editorId={item.toString()}>
                        <b>{'Thông tin người thứ ' + item.toString()}</b>
                    </Label>
                    <Field
                        key={item}
                        id={item.toString()}
...
export const ConfirmInfor = () => {
        ...
        dispatch(
            bookingSlice.actions.setOwner({
                owner: {
                    ...
                    typeUser: res.data.user.typeUser,
    ...
    if (infor.user !== undefined) {
        ...
        if (
            infor.user.typeUser === 'Foreign' ||
            bookingInfor.bookingInformation.listCustomer.some((element) => element.typeUser === 'Foreign')
        ) {
            if (bookingInfor.bookingInformation.listCustomer.length + 1 === bookingInfor.maxPeopel) {
        ...
        return (
            <div className="mt-3">
                <div>
                    <b>Người đặt phòng:</b> {infor.user.Name}
                </div>
                <div>
                    <b>Ngày nhận phòng: </b>
                    {getDayName(dateRentStart.getDay())}, ngày {dateRentStart.getDate()}, tháng{' '}
                    {dateRentStart.getMonth() + 1}, năm {dateRentStart.getFullYear()}
                </div>
                <div>
                    <b>Ngày trả phòng: </b>
                    {getDayName(dateRentEnd.getDay())}, ngày {dateRentEnd.getDate()}, tháng {dateRentEnd.getMonth() + 1}
                    , năm {dateRentEnd.getFullYear()}
                </div>
                <div className="mb-3">
                    <b>Số người trong phòng :</b> {bookingInfor.bookingInformation.amountCustomer}
                </div>
                ...
                <div>
                    <b>Tổng tiền: {totalCost.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')} VNĐ</b>
                </div>
            </div>
        );
    }
                        ...
                        <tr>
                            <th scope="col" colSpan="5">
                                <div className="d-flex table-row-header-custom">
                                    <div className="w-50">Khách hàng/Cơ quan: {bookingInforSlice.owner[0].name}</div>
                        ...
                        <tr>
                            <th scope="col" colSpan="5">
                                <div className="d-flex table-row-header-custom">
                                    ...
                                    <div className="w-50">
                                        Trị giá:{' '}
                                        {bookingInforSlice.totalCost.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}{' '}
                                        VNĐ
                                    </div>
                                </div>
                            </th>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <th className="text-center">
                                {calcDateDiff(
                                    new Date(bookingInforSlice.bookingInformation.dateRentStart),
                                    new Date(bookingInforSlice.bookingInformation.dateRentEnd),
                                )}
                            </th>
                            <th className="text-center">
